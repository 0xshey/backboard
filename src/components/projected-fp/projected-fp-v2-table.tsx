...
export function ProjectedFPV2Table({
	...
	return (
		<div className="flex flex-col gap-8">
			...
			{/* Game sections */}
			{view === "games" && gameSections.map(
				({ game, projections, standings, injurySummaries, playerInjuryStatuses }) => {
					...
					const homeStanding = homeTeam
						? standings[homeTeam.id]
						: null;
					const awayStanding = awayTeam
						? standings[awayTeam.id]
						: null;

					const homeInjuries = homeTeam
						? injurySummaries[homeTeam.id]
						: null;
					const awayInjuries = awayTeam
						? injurySummaries[awayTeam.id]
						: null;
					const hasInjuries =
						(homeInjuries?.outPlayers.length ?? 0) > 0 ||
						(awayInjuries?.outPlayers.length ?? 0) > 0;
					...
					return (
						<div key={game.id} className="flex flex-col gap-3">
							...
							{/* Injury report panel */}
							{hasInjuries && absorptionRate > 0 && (
								<div className="rounded-lg border border-amber-500/20 bg-amber-500/5 px-4 py-3 flex flex-col gap-2">
									...
									<div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
										{[awayTeam, homeTeam].map((team) => {
											if (!team) return null;
											const inj =
												injurySummaries[team.id];
											if (
												!inj ||
												inj.outPlayers.length === 0
											)
												return null;
											return (
												<div
									...
									<tbody>
										{projections.map((proj) => (
											<PlayerRows
												...
												isExpanded={
													expandedId === proj.playerId
												}
												onToggle={() =>
													toggle(proj.playerId)
												}
												colCount={
													hasCompletedGames ? 10 : 8
												}
											/>
										))}
									</tbody>
